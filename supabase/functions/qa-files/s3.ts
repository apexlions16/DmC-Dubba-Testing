...
export async function deleteObject(key:string){const s3=await client();try{await s3.send(new DeleteObjectCommand({Bucket:bucket,Key:key}));try{await s3.send(new HeadObjectCommand({Bucket:bucket,Key:key}));throw new ApiError(502,'HF kanıt dosyası silme sonrası hâlâ mevcut görünüyor.');}catch(e){if(e instanceof ApiError)throw e;}return;}catch(e){if(e instanceof ApiError)throw e;console.error('HF DeleteObject',e);throw new ApiError(502,'Kanıt dosyası Hugging Face deposundan silinemedi.');}}
