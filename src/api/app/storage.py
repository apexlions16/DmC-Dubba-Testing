...
class HfBucketStorage:
    ...
    def bug_evidence_path(self, project_id: str, report_id: str, asset_id: str, filename: str) -> str:
        return "/".join(["projects", safe_segment(project_id), "reports", safe_segment(report_id), "evidence", f"{safe_segment(asset_id)}-{safe_segment(Path(filename).name)}"])

    def retest_evidence_path(self, project_id: str, retest_id: str, asset_id: str, filename: str) -> str:
        return "/".join(["projects", safe_segment(project_id), "retests", safe_segment(retest_id), "evidence", f"{safe_segment(asset_id)}-{safe_segment(Path(filename).name)}"])

    def active_build_path(self, project_id: str, build_id: str, filename: str) -> str:
        return "/".join(["projects", safe_segment(project_id), "builds", "active", safe_segment(build_id), safe_segment(Path(filename).name)])

    def archived_build_path(self, project_id: str, build_id: str, filename: str) -> str:
        return "/".join(["projects", safe_segment(project_id), "builds", "archived", safe_segment(build_id), safe_segment(Path(filename).name)])
    ...
    def copy_file(self, source_path: str, destination_path: str) -> None:
        self.api.copy_files(f"hf://buckets/{self.bucket_id}/{source_path}", f"hf://buckets/{self.bucket_id}/{destination_path}")
